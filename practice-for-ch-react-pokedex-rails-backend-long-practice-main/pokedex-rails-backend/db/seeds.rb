...
def candidate_item 
  ...
  def create_random_item!(pokemon)
    Item.create!(
      ...
      image_url: "/images/#{find_unique_item(pokemon)}"
    )
  end
  
  ApplicationRecord.transaction do
    # These destroy_all commands are not necessary if you use `rails
    # db:seed:replant`. If they are present when you run db:seed:replant, however,
    # the command will essentially just destroy the tables twice, resulting in a
    # small increase in executuon time but no other ill effects.
    ...
    ApplicationRecord.connection.reset_pk_sequence!('pokemons')
    ApplicationRecord.connection.reset_pk_sequence!('items')
    ApplicationRecord.connection.reset_pk_sequence!('moves')
    ApplicationRecord.connection.reset_pk_sequence!('poke_moves')
   ...
   pokemon.map do |num, stats|
      poke = Pokemon.create!(
        ...
        image_url: "/images/pokemon_snaps/#{num}.svg",
        captured: !!stats['captured']
      )
  
      stats['moves'].each do |move|
        unless Move.where(name: move).present?
